...
public class BeanPropertyRenameProcessor extends RenamePsiElementProcessor {
  ...
  public void prepareRenaming(final PsiElement element, String replacementStr, Map<PsiElement, String> psiElementStringMap) {
    ...
    for (PsiClass psiClass : owners) {
      final DataBindUtil.TemplateBindingMetaData metaData = DataBindUtil.getTemplateBindingMetaData(psiClass);
      final Collection<DataBindUtil.BoundMetaData> allBoundMetaDataFromClass = DataBindUtil.getAllBoundMetaDataFromClass(psiClass);

      for (DataBindUtil.BoundMetaData md : allBoundMetaDataFromClass) {
        ...
        for (String prop : property.split("\\.")) {
          ...
          final DataBindUtil.PropertyInfo beanPropertyInfo = DataBindUtil.getBeanPropertyInfo(cls, prop.trim());
        ...
        FakeNamedPsi dummy = new FakeNamedPsi(property1) {
          ...
          public String getName() {
            final String text = org.jboss.errai.idea.plugin.Util.getAnnotationMemberValue(boundAnnotation, "property").getText();
